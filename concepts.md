...
整数可以使用十进制、十六进制(0x)和二进制(0b), 不支持使用八进制,  
正确示范如`123, +1, - 1, -0x0000fFaA, +0b10000001`  
...
此处只罗列转义字符, 转义后的结果不在此一一列出, 请去查阅相关文档  
1. `\0`
2. `\a`
3. `\b`
4. `\f`
...
实体是一或多个具有相同[基本类型](#基本类型)的值组成的数组, 可以将实体写在一对大括号中, 也可以直接写  
...
实体名称由自定义的标识符和`:`组成  
自定义的标识符不可以使用以下字符  
1. 控制字符, 及`\x0000`到`\x001f`和`\x007f`, 包括制表符, 换行符, `L'\0'`等
2. 空格 ` `
3. 及以下字符 `[` `]` `<` `>` `{` `}` `(` `)` `"` `,` `\` `/` `:`  

其余字符都可以使用, 
如`something` `未命名` `あいしてる` `str1` `1` `.-_-_-_-_-...-` 等  
...
如`[array]<i32, 3> { 1, 2, 3 }` `[array2]<2, u8> { 0, 1 }`
也可以只指明元素数量和值的类型其中一方  
...
在后置指明类型的时候, 不提供`bool`类型的指定方式, 使用[自动类型推导](#自动类型推导)已经足够.  
...
PTDS 允许不指明实体类型而靠解释器自动推导  
PTDS 实体的类型在自动推导时根据实体的第一个元素确定  
1. 可以由`i64`表示的整数, 将被推导为`i64`类型
2. 超过`i64`表示范围但在`u64`范围内的正整数, 推导为`u64`
3. 带有小数点的浮点数始终被推导为`f64`
4. 引号`""`的内容始终被推导为`str`
